...
<head>
	...
	<title>DISC Profile System | PDI Profile Questions 4</title>
	...
	If Submitted = "1" Then
		...
		If oConn.Errors.Count > 0 Then
			Response.Write "<body>" & Err.Description & "<br>Unable to record answers in database. Please try again."
			Response.End
		End If
		...
		Else
			Response.Redirect("PDIProfileQuit.asp?TCID=" & TestCodeID & "&res=" & intResellerID)
		End If
	Else
	...
	function qsubmitted() {
		...
		if(qcompleted == 1) {	
			window.location="PDIProfileQuit.asp?res=<%=intResellerID%>&TCID=<%=TestCodeID%>"
		}
	}
