...
Copyright (c) 2000-2006 SPSS Inc.
*/
  ...
  for (var i=0; (indexdoc = indexdocs[i]); i++) {
    ...
    if (initial != null) {
      ...
      for (var k=0; (e = initial.childNodes[k]); k++) {
        sortkey = index_prefix + e.getAttribute("s");
        titlenode = ts[parseInt(e.getAttribute("t")) - parseInt(ts[0].getAttribute("t"))];
...
//target is passed as key into entries associative array
  ...
  if (index_entries[e] && index_entries[e][0] == key) {
    ...
    //If multiple targets are specified, list the topics so the user can choose
      ...
      newContent += '<body onload="document.body.scrollTop=0"><h1 style="font-family:Verdana, Arial, Helvetica, sans-serif; font-size:x-small; font-weight:bold;">' + stb_select_a_topic + '</h1>\n';
...
function readEntries() {
  if (indexdocs.length == 0) {
    status = "Loading index data";
    loadXMLfile("base_index.xml",indexdocs)
    if (typeof modules_defined != 'undefined' && modules_defined) {
      for (i=0;(module=module_array[i]);i++) {
        if (eval(module[1])) {
          status = "Loading index data " + module[0].substring(7);
...
function render_search_results(results,type)
{
  ...
  resultspanel = document.getElementById("indexdiv");
    ...
    if (result != null && result.length > 0) {
      ...
      for (var i=result.length-1; i >= 0; i--) {
        if (result[i][0] != lasttopic) {
          ...
          j = i;
          var poses = new Array();
          while ((j >= 0) && (result[j][0] == result[i][0])) {
            poses.push(result[j][1]);
            j--;
          }
          entries.push([topicinfo.getAttribute("u"),topicinfo.firstChild?topicinfo.firstChild.nodeValue:"(no heading)",calculateRelevance(poses)]);
          lasttopic = result[i][0];
        }
      }
    }
  }
  ...
  entries.sort(function(a,b) { if (a[2] > b[2]) return -1; else if (a[2] == b[2]) return 0; else return 1; });
  rescaleRelevance(entries);
  for (var i=0; (entry = entries[i]); i++) {
    render_fts_hit(entry,resultspanel);
  }
  results = null;
}

function render_fts_hit(hit,container) {
  ...
  e.setAttribute("href","javascript:top.navframe.navEvent('','" + hit[0] + "?search=" + query + "');");
  ...
  e.setAttribute("class","hitcount");
  e.appendChild(document.createTextNode("["+hit[2]+"]"));
  d.appendChild(e);
...
function rescaleRelevance(entries) {
  maxrelevance = entries[0][2];
  for (i=0; (entry = entries[i]); i++) {
    entry[2] = Math.round(parseInt(entry[2])/maxrelevance * 100) + "%";
  }
}
...
function ftssearch(term,callback) {
  ...
  if (xmldocs.length == 0) {
    if (!modules_defined) {
      ...
      status = "Loading full text search data ftsdata.xml";
    }
    else {
      for (i=0; module = module_array[i]; i++) {
        if (module[1]=='true') {
          status = "Loading full text search data " + module[0].substring(7);
...
function intersect_results(a,b) {
  ...
  common_topics = intersect_topics(get_topics(a),get_topics(b));
  for (var m=0; (current = a[m]); m++) {
    if (common_topics[current[0]]) {
      result.push(current);
    }
  }
...
function intersect_topics(a,b) {
  // returns array of unique topic ID's that appear in both A and B
  if (a == null || b == null || a.length == 0 || b.length == 0) return null;
  result = new Array();
  // @@@ need to fix this so value of 0 doesn't end the for loop
  a_lim = a.length; b_lim = b.length;
  for (var i=0; i < a_lim ; i++) {
    a_target = a[i];
    for (var j=0; (j < b_lim); j++) {
      b_target = b[j];
      if (b_target > a_target) break;
      if (a_target == b_target) {
        result[a_target] = true;
      }
    }
  }
  return result;
...
function disjunct_results(a,b) {
  // returns the hits in a with the topics in b removed
  var result = new Array();
  valid_topics = disjunct_topics(get_topics(a),get_topics(b));
  if (valid_topics == null) return null;
  for (var m=0; (current = a[m]); m++) {
    if (valid_topics[current[0]]) {
      result.push(current);
    }
  }
  if (result.length == 0) return null;
...
function disjunct_topics(a,b) {
  // returns array of unique topic ID's that appear in A but not B
  if (a == null || a.length == 0) return null;
  result = new Array();
  if (b == null || b.length == 0) {
    // if nothing to delete, all a topic are valid
    for (var i=0; (a_target = a[i]); i++) {
      result[a_target] = true;
    }
    return result;
  }
  for (var i=0; (a_target = a[i]); i++) {
    result[a_target] = true;
    for (var j=0; (b_target = b[j]); j++) {
      if (b_target == a_target) {
        result[a_target] = false;
        break;
      }
    }
  }
...
function find_phrase(phrase) {
  ...
  for (var j=1; (b = wordhits[j]); j++) {
    wordhits[0] = intersect_results(wordhits[0],b);
    // if intersection is empty, don't bother with the rest
    if (wordhits[0] == null) return null;
    wordhits[j] = intersect_results(wordhits[j],wordhits[0]);
  }
  // look for instances where the words appear in consecutive positions
  result = new Array();
  for (var i=0; (wordhit = wordhits[0][i]); i++) {
    topic = wordhit[0];
    position = wordhit[1];
    found = true;
    for (var j=1; (nextword = wordhits[j]); j++) {
      if (!checktopicposition(nextword,topic,++position)) {
        found = false;
        break;
      }
    }
...
function checktopicposition(hits,topic,position) {
  ...
  for (var i=start; (hit=hits[i]); i++) {
    ...
    if (hittopic == topic) {
      if (hit[1] > position) {return false; }
      if (hit[1] == position) return true;
    }
...
function find_word(thisterm) {
  ...
  while ((low <= high) && !found) {
    ...
    word = wordlist[mid].getElementsByTagName("n")[0].firstChild.data;
    if (thisterm == word) {
      // found the term so return hits
      found = true;
      target = wordlist[mid];
    }
    else if (thisterm > word) {
      // term is in upper half of current range
      low = mid + 1;
    }
    else if (thisterm < word) {
      // term is in lower half of current range
      // if mid == high, then we're at the end and didn't find it
      high = mid - 1;
    }
  }
  ...
  // return results
  if (found) {
    hits = target.getElementsByTagName("h");
    for (i=0;(hit=hits[i]);i++) {
      hittopic = topiclist[parseInt(hit.getAttribute("t"))];
      if (!modules_defined || !hittopic.getAttribute("m") || 
      eval(hit.getAttribute("m"))) {
        poses = hit.getAttribute("p").split(",");
        for (j=0; (pos = parseInt(poses[j])); j++) {
          newresults.push([parseInt(hit.getAttribute("t")),pos]);
        }
      }
    }
  }
...
function highlightSearch() {
	...
	// in the document
	if (typeof top.frames[1].location.search != "undefined" &&
...
function runkeywordquery(query) {
  ...
  if (indexentrylists.length == 0) {
    for (i=0;(indexdoc=indexdocs[i]);i++) {
...
function keyword_find_word(thisterm) {
  ...
  hits = new Array();
  target = thisterm.toLowerCase();
  for (var i=0; (e = indexentrylist.item(i)); i++) {
    text = e.getAttribute("n");
...
function printFromTOC(id,subtopics) {
  ...
  if (!subtopics) {
    ...
    navEvent('',extractFileFromNavEvent(href));
    waitForLoad();
  }
  else {
    ...
    //@@@ and make it handle leaf TOC nodes correctly
    mainwin.loaded = false;
    navEvent('',extractFileFromNavEvent(links[firstlink].getAttribute("href")));
    prepForPrint(top.topic.document);
    //append each subsequent doc
    tempdoc = top.topic.document;
    for (i=firstlink+1; (link = links[i]); i++) {
      href = extractFileFromNavEvent(link.getAttribute("href"));
      windows.push(window.open(href,"","top=3000"));
    }
    waitForLoad();
  }
}
...
function importSubtopics() {
  ...
  var section;
  tempdoc = mainwin.topic.document;
  for (w=0; (win = windows[w]); w++) {
    prepForPrint(win.topic.document);
    p = win.topic.document.getElementById("product-icon");
    if (p) {
      p.parentNode.removeChild(p);
    }
    content = win.topic.document.body.innerHTML;
    section = tempdoc.createElement("div");
    section.innerHTML = content;
    tempdoc.body.appendChild(section);
    q = 0;
    while (q < 100000) {q++}
    setTimeout('windows['+w+'].close()',2000);
  }
  setTimeout('finishMultiPrint()',500);
}

function finishMultiPrint() {
  // after assembling all the pieces, this function
  // sends the compound document to the printer, then
  // reloads the topic
  prepForPrint(mainwin.topic.document);
  setTimeout('mainwin.topic.print();',500);
  setTimeout(mainwin.navframe.document.links[mainwin.navframe.currentLink].getAttribute("href"),1000);
}


function extractFileFromNavEvent(ne) {
  // extracts the filename part of a navEvent() call
  start = ne.indexOf(",'")+2;
  end = ne.substring(start).indexOf("'") + start;
  return ne.substring(start,end);
}

function prepForPrint(doc) {
  rt = doc.getElementById("related_topics");
  if (rt) {
    rt.parentNode.removeChild(rt);
  }
  nt = doc.getElementById("navtrail");
  if (nt) {
...
//test URL: javascript:printFromTOC("N215A2",true);
