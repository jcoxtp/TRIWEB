...
function cm_getLayerStr(m,app,name,fill,clb,arrow,ah,aw,root){
  ...
  if((!bw.reuse||l==0) && !no){
	// SPSS - kklausen : this is changed to allow onclick events for level 0 menu items
    ev=' onclick="'+name+'.showsub(\''+m.name+'\');'+name+'.onclck(\''+m.name+'\');"'
...
makeCM.prototype.showsub=function(el){
  ...
  if(c.onshow) eval(c.onshow); // SPSS, jsevaj: moved this from end of function
  ...
  x=pm.subx; y=pm.suby; b.moveIt(x,y); b.showIt()
}
/**Hide sub elements **/
