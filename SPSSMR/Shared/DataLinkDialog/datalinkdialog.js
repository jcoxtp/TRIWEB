/*
	2003-01-13 /MikkelFJ
	Added input parameter to dialog_openDialog such that it can be accessed the same
	way return value is accessed by dialog code.
	
	Added doDataLinkDialog(connectionString)	
*/

function doDataLinkDialog(connectionString, lang, index_path, initfm_file)
{
	if ( index_path==null )
		index_path = "./";
	index_path += 'index.aspx?langres=' + escape(lang);
	if(initfm_file == null)
	    initfm_file = "launch_fm.asp"
	return dialog_openDialog(index_path, '535px', '530px', { ConnectionString : connectionString, Language : lang, InitFM : initfm_file });
}

function getInputArguments()
{
	return window.dialogArguments.input_value
}
...
//****************************************************************************************
// private function
function dialog_openDialog(URL, height, width, inputValue)
{
	...
	{
		var args = {opener: window, input_value : inputValue};
		options = "resizable:yes;scroll:off;status:no;help:no;dialogHeight:" + theHeight + ";dialogWidth:" + width + ";";
		retval = window.showModalDialog(theURL,args,options);
	}
	else
	{
		...
		// is this possible? 
		// window.dialogArguments = {opener: window, input_value : inputValue};
		retval = window.dialog_return_value;
	}
	window.dialog_input_value = inputValue
	return retval
}
