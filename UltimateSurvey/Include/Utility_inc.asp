...
Function utility_sendMail(strFrom, strTo, strSubject, strBody)
   On Error Resume Next
	...
	ElseIf SV_EMAIL_OBJECT_TYPE = SV_EMAIL_JMAIL Then
		...
		'objJMail.FromName = "DISC"
		objJMail.MailServerUserName = "DISC"
		objJMail.MailServerPassword = "s3rv3r pa33word!"
		objJMail.AddRecipient CStr(strTo)
		objJMail.AddRecipient "info@triaxiapartners.com"
		objJMail.Subject = strSubject
		objJMail.Body = strBody

		'Dim strTestingMsg
		'strTestingMsg = "JMail Mail send failure. " & strFrom & " | " & strTo & " | " & SV_SMTPMAIL_HOST & " | " & strSubject & " | " & strBody

		If NOT objJMail.Send(SV_SMTPMAIL_HOST) Then
