...
function setMultiColumnRadioGroup(sourceID, colIDs, rowIDs) {
	...
	{
		cp_ctrl = findDOM(rowIDs[i], 0);
		
		if(cp_ctrl.checked)
		{
			source = findDOM(sourceID, 0);
	...
	{
		cp_ctrl = findDOM(colIDs[i], 0);
		cp_ctrl.checked = false;
	}
