...
<%
	intPageID = 37
	' We need to make sure that the TestCompleted field has been set to true before proceeding further...
	Set blnCriticalError = False
	Set oConn = CreateObject("ADODB.Connection")
	Set oCmd = CreateObject("ADODB.Command")
	With oCmd
		.CommandText = "spTestCompletedUpdate"
		.CommandType = 4
		.Parameters.Append .CreateParameter("@RETURN_VALUE", 3, 4, 0)
	...
	With oCmd
		...
		.Parameters.Append .CreateParameter("@HighPoint", 129, 3, 1, Null)
		.Parameters.Append .CreateParameter("@HighPoint2", 129, 3, 1, Null)
		.Parameters.Append .CreateParameter("@FileAlreadyExists", 3, 3, 4, Null)
		.Parameters.Append .CreateParameter("@PDFFileName", 200, 3, 50, Null)
		.Parameters.Append .CreateParameter("@TestCode", 200, 3, 50, Null)
		.Parameters.Append .CreateParameter("@PDITestSummaryID", 3, 3, 4, Null)
		.Parameters.Append .CreateParameter("@PDITestDate", 135, 3, 16, Null)
	End With
	oConn.Open strDbConnString
	oCmd.ActiveConnection = oConn
	oCmd.Execute , , 128
...
<%
	...
	Else
		...
		Response.Write "<BR>" & Request.ServerVariables("SCRIPT_NAME")
		Response.Write "<BR>" & Request.ServerVariables("HTTP_REFERRER")
		Err.Clear
	End If
%>
<!--************* E N D  R E P O R T *************-->
