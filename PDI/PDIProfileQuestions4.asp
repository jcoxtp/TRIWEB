...
' Set initial critical page parameters
	...
	intPageID = 47	' PDI Profile Questions 4 Page
%>
...
<div id="tabgraphic">
	...
	<img src="images/S1P4<%=strLanguageCode%>.gif" width="692" height="82" alt="" usemap="#tab" />
	<map name="tab">
...
<%
	...
	If Submitted = "1" Then
		...
		With oCmd
			...
			.Parameters.Append .CreateParameter("@FinalSubmit",3, 1,4, nFinalSubmit)
		End With
		...
		'Response.Write "This will take you to the next page in the profile."
		'Response.Write "<br>Your scores and highpoint will be generated here."
		'Response.End
		If Prev = "1" Then
			Response.Redirect("PDIProfileQuestions3.asp?TCID=" & TestCodeID & "&res=" & intResellerID & "&lid=" & intLanguageID)
		Else
			Response.Redirect("PDIProfileScoringSummary2.asp?TCID=" & TestCodeID & "&res=" & intResellerID & "&lid=" & intLanguageID)
		End If
	Else
	...
	' Page 4
	Words(nWord) = strTextFunLoving
	DISC(nWord) = "I"
	nWord = nWord + 1
	Words(nWord) = strTextObedient
	DISC(nWord) = "S"
	nWord = nWord + 1
	Words(nWord) = strTextTactful
	DISC(nWord) = "C"
	nWord = nWord + 1
	Words(nWord) = strTextCompetitive
	DISC(nWord) = "D"
	nWord = nWord + 1
	Words(nWord) = strTextObliging
	DISC(nWord) = "S"
	nWord = nWord + 1
	Words(nWord) = strTextAnimated
	DISC(nWord) = "I"
	nWord = nWord + 1
	Words(nWord) = strTextDecisive
	DISC(nWord) = "D"
	nWord = nWord + 1
	Words(nWord) = strTextAccurate
	DISC(nWord) = "C"
	nWord = nWord + 1
	Words(nWord) = strTextCautious
	DISC(nWord) = "C"
	nWord = nWord + 1
	Words(nWord) = strTextNeighborly
	DISC(nWord) = "S"
	nWord = nWord + 1
	Words(nWord) = strTextVigorous
	DISC(nWord) = "D"
	nWord = nWord + 1
	Words(nWord) = strTextPersuasive
	DISC(nWord) = "I"
	nWord = nWord + 1
	Words(nWord) = strTextAssertive
	DISC(nWord) = "D"
	nWord = nWord + 1
	Words(nWord) = strTextSociable
	DISC(nWord) = "I"
	nWord = nWord + 1
	Words(nWord) = strTextSteady
	DISC(nWord) = "S"
	nWord = nWord + 1
	Words(nWord) = strTextOrderly
	DISC(nWord) = "C"
	nWord = nWord + 1
	Words(nWord) = strTextReserved
	DISC(nWord) = "S"
	nWord = nWord + 1
	Words(nWord) = strTextOutspoken
	DISC(nWord) = "D"
	nWord = nWord + 1
	Words(nWord) = strTextStrict
	DISC(nWord) = "C"
	nWord = nWord + 1
	Words(nWord) = strTextEloquent
	DISC(nWord) = "I"
	nWord = nWord + 1
	Words(nWord) = strTextOutgoing
	DISC(nWord) = "I"
	nWord = nWord + 1
	Words(nWord) = strTextBold
	DISC(nWord) = "D"
	nWord = nWord + 1
	Words(nWord) = strTextModerate
	DISC(nWord) = "S"
	nWord = nWord + 1
	Words(nWord) = strTextPerfectionist
	DISC(nWord) = "C"
	nWord = nWord + 1
%>
...
<%
		...
		<tr>
			...
			<input Type="IMAGE" Name="NextPage" onclick="GoToNextPage()" SRC="images/PDISubmitAnswers.gif"></td>
		</tr>
		<% End If %>
	</table>
	</div>
	</form>
	<p class="last-page-note"><strong><%=UCase(Application("strTextNote" & strLanguageCode)) & ":</strong> " & strTextOnTheFollowingPagesYouWillSeeYour & "</p>"%>
	<script>
	<!--
	/*
	...
	If bSubmitted <> "1" Then
		...
		If oConn.Errors.Count < 1 Then
			If oRs.EOF = False Then
				oRs.MoveFirst
				While oRs.EOF = False
					If CInt(oRs("QuestionNumber")) = 37 Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 38 Then
						If oRs("Answer") = "I" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 39 Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 40 Then
						If oRs("Answer") = "S" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 41 Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 42 Then
						If oRs("Answer") = "C" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 43 Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 44 Then
						If oRs("Answer") = "D" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 45 Then
						If oRs("Answer") = "S" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 46 Then
						If oRs("Answer") = "S" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 47 Then
						If oRs("Answer") = "I" Then
						...
						End If
					ElseIf CInt(oRs("QuestionNumber")) = 48 Then
						If oRs("Answer") = "I" Then
...
<%
	...
	function qsubmitted() {
		...
		if(qcompleted == 1) {	
			window.location="PDIProfileScoringSummary2.asp?res=<%=intResellerID%>&TCID=<%=TestCodeID%>"
		}
	}
